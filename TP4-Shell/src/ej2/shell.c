...
//                 exit(127);  // Código de salida estándar para comando no encontrado
...
// #define MAX_COMMANDS 200
// #define MAX_ARGS 64 // Definido para el parseador de argumentos
...
//             // Variable para rastrear cuántos hijos se crearon exitosamente
//             // Esto es para que wait() no se llame más veces de las necesarias si un fork falla.
//             int hijos_creados_exitosamente = 0; 

//             for (int i = 0; i < command_count; i++) {
//                 if (isatty(STDIN_FILENO)) { 
//                     // printf("Command %d: %s\n", i, commands[i]); // Tu print original, si lo necesitas para depurar.
//                                                               // Lo comento porque las pruebas automatizadas podrían no quererlo.
//                 }
            
//                 pid_t pid = fork();
//                 if (pid == -1) {
//                     perror("fork");
//                     error_en_pipeline = 1; // Marcar error para no esperar a este hijo
//                     // No continuar creando más hijos para este pipeline
//                     break; 
//                 }
                
//                 hijos_creados_exitosamente++; // Incrementar solo si fork() tuvo éxito

//                 if (pid == 0) { // Proceso Hijo
//                     // Redirección de I/O
//                     if (i == 0) { // Primer comando
//                         if (dup2(pipes[i][1], 1) == -1) { perror("dup2"); exit(1); } // Usando 1 para stdout
//                     } else if (i == command_count - 1) { // Último comando
...
//                     // Cerrar todos los descriptores de pipe en el hijo (tu lógica original)
//                     for (int j = 0; j < command_count - 1; j++) {
//                         close(pipes[j][0]);
//                         close(pipes[j][1]);
//                     }

//                     // Chequear si el token del comando es vacío (ej. "cmd1 | | cmd2")
...
//                     char *args[MAX_ARGS]; // Tu array original de args
//                     int arg_count = 0;    // Tu contador original
//                     internal_parse_arguments(command_copy, args, &arg_count);

//                     if (arg_count == 0) { // Si el parseo no produjo argumentos
//                         fprintf(stderr, "Error: comando vacío en pipeline\n");
//                         exit(1);
//                     }
...
//                     fprintf(stderr, "%s: comando no encontrado\n", args[0]); // Tu error original
//                     exit(127); // Tu código de salida original
//                 }
//             } // Fin del bucle de forking

//             // Proceso Padre: cerrar todos sus descriptores de pipe (tu lógica original)
//             if (!error_en_pipeline || hijos_creados_exitosamente > 0) { // Solo cerrar pipes si se crearon algunos o no hubo error inicial
//                  for (int j = 0; j < command_count - 1; j++) {
//                     close(pipes[j][0]);
//                     close(pipes[j][1]);
//                 }
...
//             // Proceso Padre: esperar a los hijos que se crearon exitosamente
//             for (int j = 0; j < hijos_creados_exitosamente; j++) {
//                 wait(NULL);
//             }
//         }

//     end_of_command_processing:; // Etiqueta para saltar al reseteo
//         command_count = 0;
//         memset(commands, 0, sizeof(commands)); // Tu reseteo original
